...
public class ChecksumServiceTest {
    ...
    public void setUp() {
        ...
        otherList = IntStream.range(0, 50)
                .mapToObj(String::valueOf)
    ...
    public void replaceObjectsIfNotAlreadyStored() {
        List<Product> products1 = repository.saveAll(products)
                .collectList()
                .flatMap(products -> checksumService.replaceObjectsIfNotAlreadyStored(products, repository))
                .block();
    ...
    public void replaceObjectsIfNotAlreadyStoredNeg() {
        List<Product> products1 = repository.saveAll(products)
                .collectList()
                .flatMap(products -> checksumService.replaceObjectsIfNotAlreadyStored(otherList, repository))
                .flatMap(products2 -> repository.findAll()
                        .collectList())
                .block();

        assertEquals(products1, otherList);
        repository.deleteAll()
                .block();
    }
